...
# @raycast.title YouTube Analysis
...
# @raycast.argument2 { "type": "text", "optional": true, "placeholder": "URL (or will use current browser URL)" }
...
main() {
    ...
    local url=""
    local pattern="summarize"  # Default pattern

    # Handle arguments with proper checks
    if [ -n "${1:-}" ]; then
        pattern="$1"
        debug "Using provided pattern: '$pattern'"
    fi
    ...
    # Get transcript and process with pattern
    local transcript
